<?php
if ( ! class_exists( 'Blog_Articles_Social_Widget' ) ) {
	/**
	 * Adds Blog Articles Social Widget.
	 */
	class Blog_Articles_Social_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$blog_articles_social_widget = array(
				'classname'   => 'widget adore-widget social-widget style-1',
				'description' => __( 'Retrive Social Widget', 'blog-articles' ),
			);
			parent::__construct(
				'blog_articles_social_widget',
				__( 'Adore Widget: Social Widget', 'blog-articles' ),
				$blog_articles_social_widget
			);
		}
		...
		public function form( $instance ) {
			...
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'link-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Social Link %d :', 'blog-articles' ), $i ); ?></label>
