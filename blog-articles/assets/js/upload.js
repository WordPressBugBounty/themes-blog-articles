...
jQuery(function($){
  ...
  jQuery(document).on('click', 'input.select-img', function( event ){
    ...
    var Blog_Articles_Image = wp.media.controller.Library.extend({
      defaults :  _.defaults({
    ...
    blog_articles_file_frame = wp.media.frames.blog_articles_file_frame = wp.media({
      button: {
        text: jQuery( this ).data( 'uploader_button_text' ),
      },
      state : 'author-info-insert-image',
      states : [
        new Blog_Articles_Image()
        ],
          multiple: false  // Set to true to allow multiple files to be selected
        });

    // When an image is selected, run a callback.
    blog_articles_file_frame.on( 'select', function() {

      var state = blog_articles_file_frame.state('author-info-insert-image');
